...
public class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    public HttpServerHandler(HardWareUdpHandler hardWareUdpHandler) {
        this.hardWareUdpHandler = hardWareUdpHandler;
    }
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, FullHttpRequest fullHttpRequest) throws Exception{
        ...
        try {
            if (fullHttpRequest.method() == HttpMethod.GET) {
                ...
                Long cardId = Long.parseLong(params.get("cardId").toString());
                String cmd = params.get("cmd").toString();
                String data = "GET method over"+params+" > Result = ";
                if("open".equals(cmd)){
                    data+= hardWareUdpHandler.sendCommonCmd(cardId,1);
                }else if("close".equals(cmd)){
                    data+= hardWareUdpHandler.sendCommonCmd(cardId,2);
                }else if("query".equals(cmd)){
                    data+= hardWareUdpHandler.checkState(cardId);
                }
            ...
            response = responseOK(HttpResponseStatus.INTERNAL_SERVER_ERROR, copiedBuffer("INTERNAL_SERVER_ERROR", CharsetUtil.UTF_8));
        } finally {
            // 发送响应
            if(response==null) response = responseOK(HttpResponseStatus.OK, copiedBuffer("nothing", CharsetUtil.UTF_8));
